...
function visibleLevels() {
  ...
  const sorted = [...source].sort((a, b) => {
    ...
    if (sortSelect.value === "length") return String(b.length || "").localeCompare(String(a.length || ""));
...
function createAccount(form) {
  ...
  if (accounts.some((account) => account.username.toLowerCase() === username.toLowerCase())) {
    accountMessage.textContent = "This username already exists locally.";
    return;
  }

  if (pin.length < 4) {
    accountMessage.textContent = "PIN must be at least 4 symbols.";
    return;
  }
  ...
  form.reset();
  accountMessage.textContent = "Account created on this browser.";
  renderAll();
}
...
function loginAccount(form) {
  ...
  if (!account) {
    loginMessage.textContent = "Account not found on this browser.";
    return;
  }
  ...
  if (account.pin !== pin) {
    loginMessage.textContent = "Wrong PIN.";
    return;
  }
  ...
  localStorage.setItem(STORAGE_CURRENT_ACCOUNT, currentAccountId);
  form.reset();
  loginMessage.textContent = `Logged in as ${account.username}.`;
  renderAll();
}
...
function addLevelFromForm(form) {
  ...
  if (!video) {
    message.textContent = "Add a valid video link that starts with http:// or https://.";
    return;
  }
  ...
  saveStored(STORAGE_LEVELS, levels);
  form.reset();
  message.textContent = "Level added to Main List.";
  renderAll();
}
...
function renderSidebars() {
  const currentAccount = accounts.find((account) => account.id === currentAccountId);
  document.querySelector("#sideTotal").textContent = levels.length;
  document.querySelector(".side-panel small").textContent = currentAccount
    ? `${currentAccount.username} - ${currentAccount.tag}`
    : "ranked layouts";
  document.querySelector("#victorList").innerHTML = victors
    .map(
    ...
    .join("");
  document.querySelector("#legacyGrid").innerHTML = legacy
    .map(
      ([name, rank, points]) => `
...
if (submitForm) {
  submitForm.addEventListener("submit", (event) => {
    ...
    message.textContent = "Submission added to Review Queue. Accept it there to add it to the list.";
    renderQueue();
  });
}
...
if (ideaForm) {
  ideaForm.addEventListener("submit", (event) => {
    ...
    saveStored(STORAGE_IDEAS, ideas);
    event.currentTarget.reset();
    ideaMessage.textContent = "Idea sent to Management.";
    renderCommunityQueues();
  });
}
...
if (staffApplicationForm) {
  staffApplicationForm.addEventListener("submit", (event) => {
    ...
    if (form.get("role") === "Record Moderator" && !form.get("hackVerdict")) {
      staffApplicationMessage.textContent = "For Record Moderator, watch the test video and answer the hack question.";
      return;
    }
    ...
    saveStored(STORAGE_STAFF_APPLICATIONS, staffApplications);
    event.currentTarget.reset();
    staffApplicationMessage.textContent = "Application sent to Management.";
    renderCommunityQueues();
  });
}
...
if (rankRequestForm) {
  rankRequestForm.addEventListener("submit", (event) => {
    ...
    saveStored(STORAGE_RANK_REQUESTS, rankRequests);
    event.currentTarget.reset();
    rankRequestMessage.textContent = "Rank change request sent to Management.";
    renderCommunityQueues();
  });
}
...
if (recordSubmitForm) {
  recordSubmitForm.addEventListener("submit", (event) => {
    ...
    if (!video) {
      recordSubmitMessage.textContent = "Add a valid video link that starts with http:// or https://.";
      return;
    }
    ...
    saveStored(STORAGE_RECORD_QUEUE, recordQueue);
    event.currentTarget.reset();
    recordSubmitMessage.textContent = "Record added to Record Queue.";
    renderRecordQueue();
  });
}
